...
The system MUST accept `POST /generate` requests with `workflow = "realistic_persona"` and declared persona controls: `age` (integer, 18-100), `gender` (non-empty string), `ethnicity` (non-empty string), `wardrobe` (non-empty string), `expression` (non-empty string), and `background` (non-empty string). The system MUST reject requests with undeclared controls, empty string values, or values outside declared ranges. Omitted optional persona controls MUST fall back to workflow manifest defaults. The system MUST return `202 Accepted` with `job_id` and `status = "pending"`.
