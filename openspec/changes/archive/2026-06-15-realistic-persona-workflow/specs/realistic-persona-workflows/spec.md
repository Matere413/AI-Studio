...
The system MUST use `moodyRealMix_zitV7.safetensors` as the default checkpoint for `realistic_persona`. The checkpoint identifier MUST be configurable for future validation. The workflow MUST NOT include FaceDetailer, IPAdapter, InstantID, or reference-image identity preservation in V1.

#### Scenario: Default checkpoint applied

- GIVEN a `realistic_persona` request without explicit checkpoint override
- WHEN the workflow executes
- THEN `moodyRealMix_zitV7.safetensors` is used as the generation checkpoint
