...
## Phase 3: Identity Flow & Legacy Cleanup (PR 3) ✅ COMPLETE

- [x] 3.1 Create `api/src/shared/flows/identity.py` — `IdentityRequest(BaseAtomicFlow)` with `reference_face: ImageArtifact`, optional `seed`, `width`/`height` (multiples of 64); `IdentityFlow` binding `workflow_name="identity"`, `gpu_profile=A100`, `timeout_s=1200`
- [x] 3.2 Modify `api/src/shared/modal_config.py` — add `ComfyUI-PuLID-Flux` node already present; verify PuLID model in whitelist; add A100 GPU function reference
- [x] 3.3 Create `api/src/workflows/identity/manifest.yaml` — inputs: `prompt`, `reference_face` → LoadImage, `seed`, `pulid`, `unet`, `clip`, `vae`, `face_detector`; defaults for model filenames
- [x] 3.4 Create `api/src/workflows/identity/workflow.json` — ComfyUI graph: LoadImage(face) → PuLIDModelLoader → ApplyPuLID; CLIPTextEncode → KSampler(FLUX UNet) → VAEDecode → FaceDetailer → SaveImage
- [x] 3.5 Modify `api/src/features/generation/modal_tasks.py` — add `run_generation_a100` Modal function (A100, timeout=3600s)
- [x] 3.6 Modify `api/src/features/generation/service.py` — register `IDENTITY_FLOW`; remove `IDENTIDAD_GGUF_WORKFLOW` from `SUPPORTED_WORKFLOWS`; remove `identidad_gguf` branching from `enqueue_modal_work`; remove `resolve_identity_seed` and `download_image_to_base64`
- [x] 3.7 Modify `api/src/features/generation/router.py` — add `POST /generate/identity` route accepting `IdentityFlow`, returning 202
- [x] 3.8 Modify `api/src/features/generation/models.py` — remove `identidad_gguf` from `WorkflowName` literal and `SUPPORTED_WORKFLOWS`; remove `image_url`, `width`, `height`, `seed` workflow-scoped validators
- [x] 3.9 Delete `api/src/workflows/identidad_gguf/` directory (workflow.json + manifest.yaml)
- [x] 3.10 Update `api/src/tests/test_generation_models.py` — remove GGUF workflow tests; add rejection tests for identidad_gguf
- [x] 3.11 Update `api/src/tests/test_generation_service.py` — remove GGUF branching tests; add `dispatch_flow` tests for identity flow (A100 routing, params, timeout_s)
- [x] 3.12 Create `api/src/tests/test_identity_flow.py` — 30 tests: IdentityRequest validation (dimensions, media_type, seed), IdentityFlow binding, workflow assets contract tests
- [x] 3.13 Update `api/src/tests/test_workflow_templates.py` — remove `identidad_gguf` template test; add extraction, composition, identity template validation tests
- [x] 3.14 Update `api/src/tests/test_modal_config.py` — verify BRIA, ControlNet, PuLID, Impact-Pack nodes in image build commands

## All Phases Complete ✅

All 14 Phase 3 tasks completed. 360 tests passing.
3-chained PR delivery: `pr1` (Extraction) → `pr2` (Composition) → `pr3` (Identity + cleanup).
